...
const palettes: any = {
  ...
  const ColorPalette = () => {
    return Object.keys(palettes).map(palette => {
      return <div className="flex justify-between mb-3 border-b pb-3" key={palette}>
        <div className="text-sm"><code>{palette}</code></div>
        <div className="flex flex-wrap text-xs select-none">
          {palettes[palette]['colorSet'].map((color: string) => {
            return <div key={`${palette}-${color}`} style={{backgroundColor: color, color: color}}>___<span className="hidden sm:inline">..</span></div>
          })}
        </div>
      </div>
    })
